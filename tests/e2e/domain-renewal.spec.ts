...
test.describe("Domain Renewal", () => {
  ...
  test("should show wallet connection prompt when not connected", async ({
    ...
    await expect(connectWalletButton)
  ...
  test.describe("with wallet connected", () => {
    test.beforeEach(async ({ page }) => {
      await gotoAndRestoreWallet(page, RENEWAL_URL);
    });
    ...
    test("should have URL matching /domain/{name}/renew pattern", async ({
      page,
    }) => {
      await expect(page).toHaveURL(/\/domain\/name\.mpc\/renew/);
    });

    test("should display year selector with increment button", async ({
      page,
    }) => {
      const incrementButton = page.locator(
        `button[aria-label="${TEXT.ADD_YEAR}"]`,
      );
      await expect(incrementButton).toBeVisible();
    });

    test("should display year selector with decrement button", async ({
      page,
    }) => {
    ...
    test("should display Go back button", async ({ page }) => {
      const goBackButton = page
        .getByRole("button", { name: TEXT.GO_BACK })
        .or(page.locator(`a[href*="/domain/${TEST_DOMAIN_NAME}"]`));
    ...
    test("should display total cost that updates with year selection", async ({
      page,
    }) => {
    ...
    test("should display proceed to payment or renew button", async ({
      page,
    }) => {
      const renewButton = page
        .getByRole("button", { name: TEXT.RENEW })
    ...
    test("should display domain name being renewed", async ({ page }) => {
      const domainName = page.getByText(TEST_DOMAIN_NAME).first();
      await expect(domainName).toBeVisible();
    });

    test("should navigate back to domain page when Go back is clicked", async ({
      page,
    }) => {
      const goBackButton = page
        .getByRole("button", { name: TEXT.GO_BACK })
    ...
    test("should display year selector with default value of 1 year", async ({
      page,
    }) => {
      const yearDisplay = page.getByText(/1\s*year/i).first();
