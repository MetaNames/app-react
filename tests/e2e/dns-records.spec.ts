...
test.describe("DNS Records Management", () => {
  ...
  test("non-owner cannot see records editor in settings tab", async ({
    page,
  }) => {
  ...
  test("owner can view records container in settings tab", async ({ page }) => {
    ...
    if (await skipIfNotOwner(page)) {
      test.skip(
        true,
        `Wallet does not own ${TEST_DOMAIN} - settings tab not visible`,
      );
    }
  ...
  test("add-record form has record type dropdown", async ({ page }) => {
    ...
    const recordTypeTrigger = page
      .locator(`${SELECTORS.ADD_RECORD_FORM} button[role="combobox"]`)
      .first();
    await expect(recordTypeTrigger).toBeVisible();
  });

  test("record type dropdown shows available record types", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
  ...
  test("Add record button is disabled when no record type selected", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
  ...
  test("Add record button is disabled when record value is empty", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
  ...
  test("Add record button is enabled when record type and value are filled", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
  ...
  test("validation error shows when record value exceeds 64 characters", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
    ...
    const charCount = page.locator(
      `${SELECTORS.ADD_RECORD_FORM} >> text=/\\d+\\/64/`,
    );
  ...
  test("validation error shows for invalid URL on Uri type", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
    ...
    const errorMsg = page.locator(
      `${SELECTORS.ADD_RECORD_FORM} .${CSS_CLASSES.TEXT_DESTRUCTIVE}`,
    );
    await expect(errorMsg).toContainText(VALIDATION_MESSAGES.INVALID_URL);
  });

  test("validation error shows for invalid email on Email type", async ({
    page,
  }) => {
    if (!(await gotoAndRestoreWallet(page, `/domain/${TEST_DOMAIN}`))) {
      test.skip(true, "Wallet not available");
  ...
  test("record value textarea shows character count", async ({ page }) => {
    ...
    const charCount = page.locator(
      `${SELECTORS.ADD_RECORD_FORM} [class*="char-count"], ${SELECTORS.ADD_RECORD_FORM} [class*="counter"]`,
    );
