...
test.describe("TLD Information", () => {
  ...
  test("should show loading spinner while fetching TLD data", async ({
    page,
  }) => {
  ...
  test("should display owner chip linking to contract page", async ({
    page,
  }) => {
    await expect(page.locator(SELECTORS.DOMAIN_TITLE)).toBeVisible({
      timeout: SPINNER_TIMEOUT_MS,
  ...
  test("should NOT display Settings tab for TLD (isTld=true)", async ({
    page,
  }) => {
    await expect(page.locator(SELECTORS.DOMAIN_TITLE)).toBeVisible({
      timeout: SPINNER_TIMEOUT_MS,
  ...
  test("should NOT display Details tab for TLD (isTld=true)", async ({
    page,
  }) => {
    await expect(page.locator(SELECTORS.DOMAIN_TITLE)).toBeVisible({
      timeout: SPINNER_TIMEOUT_MS,
