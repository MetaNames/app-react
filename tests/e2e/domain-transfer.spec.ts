...
test.describe("Domain Transfer", () => {
  test.describe("with wallet connected", () => {
    ...
    test("should display transfer page with all elements", async ({ page }) => {
      ...
      await expect(
        warningDiv.locator(
          'strong:has-text("Please note that all transfers are irreversible.")',
        ),
      ).toBeVisible();
      await expect(
        warningDiv.locator('strong:has-text("Verify the address is correct")'),
      ).toBeVisible();

      const addressInput = page.getByPlaceholder(
        PLACEHOLDERS.RECIPIENT_ADDRESS,
      );
    ...
    test("should clear validation error when address becomes valid", async ({
      page,
    }) => {
      const addressInput = page.getByPlaceholder(
        PLACEHOLDERS.RECIPIENT_ADDRESS,
      ...
      await expect(addressInput).toHaveClass(/(^| )border-destructive( |$)/);
      await expect(
        page.locator(`p:has-text("${TEXT.ADDRESS_INVALID}")`),
      ).toBeVisible();

      await addressInput.fill(VALID_ADDRESS);
      await expect(addressInput).not.toHaveClass(
        /(^| )border-destructive( |$)/,
      );
      await expect(
        page.locator(`p:has-text("${TEXT.ADDRESS_INVALID}")`),
    ...
    test("should show transfer button when wallet is connected", async ({
      page,
    }) => {
    ...
    test("should disable transfer button for invalid address when connected", async ({
      page,
    }) => {
      const addressInput = page.getByPlaceholder(
        PLACEHOLDERS.RECIPIENT_ADDRESS,
    ...
    test("should enable transfer button for valid address when connected", async ({
      page,
    }) => {
      const addressInput = page.getByPlaceholder(
        PLACEHOLDERS.RECIPIENT_ADDRESS,
  ...
  test("should hide transfer button when wallet not connected", async ({
    page,
  }) => {
