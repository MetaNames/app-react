...
describe('connectDevPrivateKey', () => {
  ...
  it('imports private key and returns address', async () => {
    const privateKey = 'df4642ef258f9aef2adb6c148590208b20387fb067f2c0907d6c85697c27928c';
    const address = await connectDevPrivateKey(mockSdk as MetaNamesSdk, privateKey);
  ...
  it('calls setSigningStrategy with privateKey strategy', async () => {
    ...
    expect(mockSdk.setSigningStrategy).toHaveBeenCalledWith('privateKey', privateKey);
  });
});
