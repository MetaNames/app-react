...
describe('validateRecordValue', () => {
  ...
  describe('Email type', () => {
    ...
    it('returns error for invalid email format', () => {
      expect(validateRecordValue('Email', 'notanemail')).toBe('Must be a valid email');
    });

    it('returns error for email without @', () => {
      expect(validateRecordValue('Email', 'testexample.com')).toBe('Must be a valid email');
    });

    it('returns error for email without domain', () => {
      expect(validateRecordValue('Email', 'test@')).toBe('Must be a valid email');
    });
  });
  ...
  describe('Uri type', () => {
    ...
    it('returns error for invalid URL', () => {
      expect(validateRecordValue('Uri', 'not a url')).toBe('Must be a valid URL');
    });

    it('returns error for URL without protocol', () => {
      expect(validateRecordValue('Uri', 'example.com')).toBe('Must be a valid URL');
    });
  });
  ...
  describe('Wallet type', () => {
    ...
    it('accepts any text (current behavior - no blockchain address format validation)', () => {
      ...
      expect(validateRecordValue('Wallet', 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh')).toBeNull();
    });
  });
  ...
  describe('Avatar type', () => {
    it('accepts valid URL', () => {
      expect(validateRecordValue('Avatar', 'https://example.com/avatar.png')).toBeNull();
    });

    it('returns error for invalid URL', () => {
      expect(validateRecordValue('Avatar', 'not-a-url')).toBe('Must be a valid URL');
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('rejects 65 character string for Bio', () => {
      expect(validateRecordValue('Bio', 'a'.repeat(65))).toBe('Max 64 characters');
    });
  ...
  describe('Email validation edge cases', () => {
    ...
    it('rejects email with spaces', () => {
      expect(validateRecordValue('Email', 'user @example.com')).toBe('Must be a valid email');
    });
    ...
    it('rejects email starting with @', () => {
      expect(validateRecordValue('Email', '@example.com')).toBe('Must be a valid email');
    });
  });
  ...
  describe('URL validation edge cases', () => {
    ...
    it('accepts URL with query params', () => {
      expect(validateRecordValue('Uri', 'https://example.com/path?query=value')).toBeNull();
    });

    it('accepts URL with fragment', () => {
      expect(validateRecordValue('Uri', 'https://example.com/path#section')).toBeNull();
    });
    ...
    it('rejects URL with only path', () => {
      expect(validateRecordValue('Uri', '/path/to/resource')).toBe('Must be a valid URL');
    });
  });
