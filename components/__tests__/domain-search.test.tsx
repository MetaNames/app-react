...
vi.mock('@/lib/domain-validator', () => ({
  ...
  normalizeDomain: vi.fn((name: string) => name.endsWith('.mpc') ? name : `${name}.mpc`),
}));
...
describe('DomainSearch', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (useSdkStore as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ metaNamesSdk: mockMetaNamesSdk });
  ...
  describe('Validation', () => {
    ...
    it('shows validation error for invalid domain (special chars)', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Domain name can only contain letters, numbers, and hyphens')).toBeInTheDocument();
      });
    });
  });
