    ...
    <Card className="w-full max-w-lg">
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <Chip label="parent" value={parentDomain} href={`/domain/${parentDomain}`} />
        </div>
        <div className="flex items-center justify-between py-2 border-t">
          <span className="text-muted-foreground">Price</span>
          <span className="font-bold text-[hsl(var(--chip-available-fg))]">FREE</span>
        </div>
        <LoadingButton onClick={handleRegister} className="w-full">Register domain</LoadingButton>
      </CardContent>
    </Card>
  );
}