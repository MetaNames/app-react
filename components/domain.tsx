...
function JdenticonAvatar({ value, size = 64 }: { value: string; size?: number }) {
  const ref = useRef<SVGSVGElement>(null);
  useEffect(() => {
    import('jdenticon').then(({ update }) => { if (ref.current) update(ref.current, value); });
  }, [value]);
    ...
    <div className="flex flex-col gap-6">
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {profileRecords.map((type) => {
              ...
              if (type === 'Uri') return <Chip key={type} label="link" value={val} href={val} />;
              return <Chip key={type} label={type.toLowerCase()} value={type === 'Price' ? `${val} $` : val} />;
            })}
            <Chip label="link" href={shortLinkUrl(domain.name)} value={shortLinkUrl(domain.name)} />
          </div>
        </section>
      )}
      <section>
      ...
      {socialRecords.length > 0 && (
        <section>
          ...
          <div className="flex flex-wrap gap-2">{socialRecords.map((type) => <Chip key={type} label={type.toLowerCase()} value={domain.records[type]} />)}</div>
        </section>
      )}
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-2xl">
      <div className="flex items-center gap-4">
        <div className="avatar"><JdenticonAvatar value={domain.name} size={64} /></div>
        <div>
          <h5 className="domain text-2xl font-bold" data-testid="domain-title">{domain.name}</h5>
        ...
        <Tabs defaultValue="details">
          ...
          <TabsContent value="settings" className="mt-4 flex flex-col gap-4">
            ...
            <div className="flex gap-3 pt-4 border-t">
              <Button variant="outline" onClick={() => router.push(`/domain/${domain.name}/renew`)}>Renew</Button>
              <Button variant="outline" onClick={() => router.push(`/domain/${domain.name}/transfer`)}>Transfer</Button>
            </div>
          </TabsContent>
        </Tabs>
      ) : <DetailsContent />}
