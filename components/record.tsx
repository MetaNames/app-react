      ...
      <div className="flex-1 min-w-0">
        ...
        {editing ? (
          <div className="mt-1 flex flex-col gap-1">
            <Textarea value={editValue} onChange={(e) => { setEditValue(e.target.value); setEditError(null); }} rows={2} maxLength={64} />
          ...
          <p className="mt-0.5 truncate">
            {isUrlRecord(type) ? <a href={value} target="_blank" rel="noopener noreferrer" className="hover:underline">{value}</a> : value}
            {type === 'Price' && ' $'}
          </p>
        )}
      </div>
